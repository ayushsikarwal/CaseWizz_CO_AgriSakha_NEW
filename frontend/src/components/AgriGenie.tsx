...
const AgriGenie: React.FC = () => {
  ...
  const [results, setResults] = useState<AgriGenieResult | null>(null);
  ...
  const fetchResults = async (selectedRegion: string, selectedCrop: string) => {
    ...
    hideError();
    setResults(null);
    setActiveAdviceTab(null); // Reset active tab on new search

    try {
      const response = await fetch('http://localhost:5000/agri_analyze', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setResults(data);
        setActiveAdviceTab('irrigation'); // Default to irrigation advice after results
      } else {
  ...
  console.log("Seed varieties data:", results?.seed_varieties);
  
  // Additional debugging for seed varieties
  if (results?.seed_varieties) {
    console.log("🔍 [FRONTEND] Seed varieties type:", typeof results.seed_varieties);
    console.log("🔍 [FRONTEND] Seed varieties keys:", Object.keys(results.seed_varieties));
    console.log("🔍 [FRONTEND] Weather adapted choice:", results.seed_varieties.weather_adapted_choice);
    console.log("🔍 [FRONTEND] Top 5 varieties count:", results.seed_varieties.top_5_varieties?.length);
    console.log("🔍 [FRONTEND] Best in region:", results.seed_varieties.best_in_region);
  } else {
    console.log("❌ [FRONTEND] No seed varieties data found");
  }

  // Additional debugging for comprehensive advice
  if (results?.comprehensive_advice) {
    console.log("🔍 [FRONTEND] Comprehensive advice type:", typeof results.comprehensive_advice);
    console.log("🔍 [FRONTEND] Comprehensive advice keys:", Object.keys(results.comprehensive_advice));
    console.log("🔍 [FRONTEND] Pest management count:", results.comprehensive_advice.pest_disease_management?.length);
    console.log("🔍 [FRONTEND] Fertilizer recommendations count:", results.comprehensive_advice.fertilizer_recommendations?.length);
  } else {
    console.log("❌ [FRONTEND] No comprehensive advice data found");
  }
                ...
                <div className="bg-white/10 backdrop-blur-lg rounded-xl shadow-lg p-4 text-white">
                  {showDailyWeather ? (
                    ...
                    <div className="px-4 pb-4 pt-0">
                        ...
                        {/* MODIFIED: Changed from grid to a horizontally scrollable flexbox */}
                        <div className="flex justify-center overflow-x-auto space-x-6 text-center">
                            {results.forecast.map((day, index) => (
                            // MODIFIED: Added flex-shrink-0 and a fixed width (w-36)
                ...
                <div className="grid grid-cols-1 lg:grid-cols-1">
                  ...
                  <div className="bg-white/10 backdrop-blur-lg rounded-xl shadow-lg p-8 text-white flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar" style={{ maxHeight: '1800px' }}>
                      {activeAdviceTab === 'irrigation' && results?.irrigation_advice && (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              <div>
                                ...
                                <ul className="list-disc list-inside text-gray-400 space-y-1">
                                  {results.irrigation_advice.monitoring_guidelines.map((item, idx) => (
                                    <li key={idx}>{item}</li>
                                  ))}
                                </ul>
                              </div>
                              <div>
                                ...
                                <ul className="list-disc list-inside text-gray-400 space-y-1">
                                  {results.irrigation_advice.key_factors.map((item, idx) => (
                                    <li key={idx}>{item}</li>
                                  ))}
                                </ul>
                              </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {results.irrigation_advice.daily_advice.map((day, index) => (
                          ...
                          <div className="bg-gray-800 p-6 rounded-lg shadow-inner border border-gray-700">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {results.irrigation_advice.action_legend.map((item, idx) => (
                      ...
                      {activeAdviceTab === 'seed' && results?.seed_varieties && (
                        <div className="space-y-8">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                              {results.seed_varieties.top_5_varieties.map((seed, index) => (
                                <div key={index} className="bg-gray-800 rounded-lg shadow-lg p-5 border border-gray-700 flex flex-col justify-between">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-700">
                              ...
                              <ul className="list-none p-0 space-y-1">
                                {results.seed_varieties.best_in_region.features.map((feature, idx) => (
                                  <li key={idx} className="flex items-center text-green-400"><CheckCircle className="w-4 h-4 mr-2" /> {feature}</li>
                                ))}
                              </ul>
                            </div>
                      ...
                      {activeAdviceTab === 'ai' && results?.comprehensive_advice && (
                        <div className="space-y-8 bg-transparent p-6 rounded-lg">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {results.comprehensive_advice.pest_disease_management.map((pest, index) => (
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="space-y-4">
                              {results.comprehensive_advice.fertilizer_recommendations.map((stage, index) => (
...
/* Inject global styles if needed, or rely on tailwind.css / index.css */
/* The previous inline style injections are removed as they are not standard React practice for complex layouts */
/* You should ensure tailwind CSS is properly configured and imported in index.css or App.tsx for utility classes */
